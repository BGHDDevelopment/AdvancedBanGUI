...
public class Items {
    ...
    public static ItemStack Kick(Player p) {
        ItemStack stone = new ItemStack(Material.getMaterial(Main.plugin.getguiitems1Config().getString("KickItem")));
        ItemMeta stonem = stone.getItemMeta();
        stonem.setDisplayName(ChatColor.translateAlternateColorCodes('&', Main.plugin.getguiitems1Config().getString("KickName")));
        ArrayList<String> lore = new ArrayList<>();
        List<String> stringList = Main.plugin.getguiitems1Config().getStringList("KickLore");
        IntStream.range(0, stringList.size()).forEach(i -> lore.add(getColor(stringList.get(i))));
        stonem.setLore(lore);
        stone.setItemMeta(stonem);
        return stone;
